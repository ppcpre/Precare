...
export function AppointmentReceipts({
  ...
  function onRead(read: ReceiptRead, suggested: number | null) {
    ...
    if (read.status === "not_found") {
      ...
      /**
       * verified === false = รายการย่อยที่อ่านได้บวกกันไม่เท่ายอดรวม
       * แปลว่าน่าจะอ่านผิดสักตัว ยังเติมให้ตรวจได้ แต่ต้องเตือนให้ต่างจากปกติ
       * (วัดมาแล้วว่าโมเดลตอบเลขผิดอย่างมั่นใจได้ และผิดซ้ำเดิมทุกรอบ)
       */
      setNotice(
        read.verified === false
          ? "ตัวเลขในใบเสร็จนี้อ่านแล้วไม่สอดคล้องกัน (รายการย่อยบวกกันไม่เท่ายอดรวม) — ตรวจกับใบเสร็จให้ดีก่อนบันทึก"
          : null,
      );
