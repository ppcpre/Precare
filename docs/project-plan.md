...
🔴 **ยังเตือนอัตโนมัติไม่ได้** — ยังไม่มีตัวจับเวลาที่คอยเช็คว่านัดไหนใกล้ถึงแล้วยิง push
ตอนนี้มีแต่ปุ่ม "ส่งทดสอบ" ที่ผู้ใช้กดเอง ตัวจับเวลาต้องใช้ Cron Trigger
ซึ่ง OpenNext คุม entry point ของ worker อยู่ อาจต้องแยก worker เล็กอีกตัว

🟡 **ต้องตั้ง secret ก่อนใช้ได้** `npx wrangler secret put VAPID_PRIVATE_KEY --env dev`
