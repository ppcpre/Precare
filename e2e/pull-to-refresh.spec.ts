...
test("ลากลงเพื่อโหลดหน้าใหม่ — และท่าที่ไม่ควรรีเฟรช", async ({ page }) => {
  ...
  await test.step("เลื่อนหน้าลงไปแล้ว ลากลงคือเลื่อนกลับขึ้น ไม่ใช่รีเฟรช", async () => {
     ...
     *
     * เทียบกับ 0 ไม่ใช่เลขตายตัว เพราะด่านที่กำลังทดสอบคือ `window.scrollY > 0`
     * ใน PullToRefresh เอง ไม่ใช่ "เลื่อนได้กี่พิกเซล" — ความยาวหน้าแรกเปลี่ยนได้
     * ทุกครั้งที่แก้ระยะห่างของการ์ด เคยตั้งไว้ > 100 แล้ว CI ได้ 100 พอดีจนแดง
     * หลังลดระยะห่างระหว่างการ์ด
     *
     * ถ้าวันหนึ่งหน้าแรกสั้นจนเลื่อนไม่ได้เลย poll นี้จะแดงเองพร้อมข้อความ
     * ซึ่งถูกต้อง เพราะเทสต์นี้จะไม่ได้ทดสอบอะไรอีกต่อไป
     */
    await expect
      .poll(
        () =>
        ...
        { message: "หน้าแรกต้องเลื่อนลงได้จริง ไม่งั้นเทสต์นี้ไม่ได้ทดสอบด่าน scrollY > 0" },
      )
      .toBeGreaterThan(0);
